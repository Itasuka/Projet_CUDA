...
__global__ void gaussian5x5( unsigned char * rgb, unsigned char * rgb2, std::size_t cols, std::size_t rows ) {
  auto i = blockIdx.x * blockDim.x + threadIdx.x;
  auto j = blockIdx.y * blockDim.y + threadIdx.y;
  
    if( i>2 && i < cols-2 && j>1 && j < rows-2 ) {
  
      for(int k=0; k<3; ++k){
          auto res =                          
                rgb[3*((j-2)*cols+i-2)+k] + 4*  rgb[3*((j-2)*cols+i-1)+k] + 6*  rgb[3*((j-2)*cols+i)+k] + 4*  rgb[3*((j-2)*cols+i+1)+k] +     rgb[3*((j-2)*cols+i+2)+k] +
            4 * rgb[3*((j-1)*cols+i-2)+k] + 16* rgb[3*((j-1)*cols+i-1)+k] + 24* rgb[3*((j-1)*cols+i)+k] + 16* rgb[3*((j-1)*cols+i+1)+k] + 4*  rgb[3*((j-1)*cols+i+2)+k] +
            6 * rgb[3*(j*cols+i-2)+k]     + 24* rgb[3*(j*cols+i-1)+k]     + 36* rgb[3*(j*cols+i)+k]     + 24* rgb[3*(j*cols+i+1)+k]     + 6*  rgb[3*(j*cols+i+2)+k] +
            4 * rgb[3*((j+1)*cols+i-2)+k] + 16* rgb[3*((j+1)*cols+i-1)+k] + 24* rgb[3*((j+1)*cols+i)+k] + 16* rgb[3*((j+1)*cols+i+1)+k] + 4*  rgb[3*((j+1)*cols+i+2)+k] +
                rgb[3*((j+2)*cols+i-2)+k] + 4*  rgb[3*((j+2)*cols+i-1)+k] + 6*  rgb[3*((j+2)*cols+i)+k] + 4*  rgb[3*((j+2)*cols+i+1)+k] +     rgb[3*((j+2)*cols+i+2)+k];
                                       
          rgb2[ 3 *(j * cols + i) + k ] = res/256;                                
      }
...
int main()
{
  ...
  gaussian5x5<<< grid0, block >>>( rgb_d, rgb2_d, cols, rows * 3 ); 
  ...
  cv::imwrite( "outGaussian_cu.jpg", m_out );
